...
import { Task5ChildComponent } from "../task5ChildComponent/task5-child-component";
import { Task5Service } from '../../../services/task5Service/task5-service';

@Component({
  selector: 'app-task5-parent-component',
  imports: [Task5ChildComponent],
  templateUrl: './task5-parent-component.html',
  styleUrl: './task5-parent-component.scss',
})
...
users: any[] = [];
  ...
  newUserName: string = '';
  constructor(private task5service: Task5Service, private cdr: ChangeDetectorRef) { }
  ...
  getUsers() {
    ...
    // });
    this.task5service.getData().subscribe({
      next:(data)=>{
        this.isLoading=false;
        this.isError=false;
        this.users=data;
